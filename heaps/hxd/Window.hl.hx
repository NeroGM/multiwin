	...
	function onEvent( e : #if hldx dx.Event #else sdl.Event #end ) : Bool {
		...
		case WindowState:
			...
			case Focus:
				...
				#end
				wasBlurred = false;
				event(new Event(EFocus,e.winId));
			case Blur:
				wasBlurred = true;
				event(new Event(EFocusLost,e.winId));
				#if hldx
				// release all keys
			...
			case Enter:
				...
				#end
				event(new Event(EOver, e.winId));
			case Leave:
				event(new Event(EOut, e.winId));
			case Close:
				onClose();
			default:
			}
		case MouseDown if (!hxd.System.getValue(IsTouch)):
			curMouseX = e.mouseX;
			curMouseY = e.mouseY;
			eh = new Event(EPush, e.winId, e.mouseX, e.mouseY);
		...
		case MouseUp if (!hxd.System.getValue(IsTouch)):
			curMouseX = e.mouseX;
			curMouseY = e.mouseY;
			eh = new Event(ERelease, e.winId, e.mouseX, e.mouseY);
			eh.button = switch( e.button - 1 ) {
			case 0: 0;
			case 1: 2;
			case 2: 1;
		...
		case MouseMove if (!hxd.System.getValue(IsTouch)):
			curMouseX = e.mouseX;
			curMouseY = e.mouseY;
			eh = new Event(EMove, e.winId, e.mouseX, e.mouseY);
		case MouseWheel:
			eh = new Event(EWheel, e.winId, mouseX, mouseY);
		...
		case TextInput:
			eh = new Event(ETextInput, e.winId, mouseX, mouseY);
